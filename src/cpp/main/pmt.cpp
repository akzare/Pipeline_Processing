...
pmt_t string_to_symbol(const std::string& name)
{
  ...
  std::lock_guard<std::mutex> locker(thread_safety);
...
bool is_blob(pmt_t x) { return is_genVector<uint8_t>(x); }

pmt_t make_blob(const void* buf, size_t len_in_bytes)
{
  return init_genVector<uint8_t>(len_in_bytes, (const uint8_t*)buf);
}
